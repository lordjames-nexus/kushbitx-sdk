...
const serviceName = (process.argv.find(value => value.startsWith('--service='))?.split('=')[1] || 'token-risk').toLowerCase();
const aliases = { token: 'token-risk', 'token-risk': 'token-risk', preflight: 'transaction-preflight', 'transaction-preflight': 'transaction-preflight', payment: 'verify-payment', 'payment-proof': 'verify-payment', 'verify-payment': 'verify-payment' };
const selectedService = aliases[serviceName];
if (!selectedService) throw new Error('Unknown service. Use token-risk, preflight or payment.');
const recoveryFile = process.env.RECOVERY_FILE || (selectedService === 'token-risk' ? 'kushbitx-recovery.json' : 'kushbitx-' + selectedService + '-recovery.json');
const reportFile = process.env.REPORT_FILE || (selectedService === 'token-risk' ? 'kushbitx-report.json' : 'kushbitx-' + selectedService + '-report.json');
...
function serviceConfig(accountAddress) {
  if (selectedService === 'token-risk') return { path: '/api/token-risk', amount: '250000', price: '0.25', input: { chain: 'base', address: process.env.TOKEN_ADDRESS || asset } };
...
async function consume(response) {
  ...
  // No trade, transaction or payment is executed from a result by this example.
}
...
async function main() {
  ...
  // Exclusive creation prevents overwriting an unresolved order's recovery key.
  ...
  const authorization = Object.fromEntries(Object.entries(message).map(([key, value]) => [key, typeof value === 'bigint' ? value.toString() : value]));
  ...
  // An interrupted call is recovered; this example never creates a second signature automatically.
